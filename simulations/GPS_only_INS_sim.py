...
from filters.filter_covariance import INS_filter_covariance, INS_EqF_covariance
...
class GPS_only_INS_sim(sim):
    ...
    Callable interface:
     - sim_run(run_id, enable_replay)
       ...
       + return result_data dict as configured
         ...
         - waypose_steps - time steps at which robots arrived at wayposes 
            + included if config.precompute_robot_trajectories is set to False
            + result_data['waypose_steps'][robot_idx][waypose_idx]
         ...
         - e_N - final filter estimation error for each robot and filter
            + included if config.record_final_error is set to True 
            + result_data['e_N'][robot_idx][filter_idx][var]
         - e_avg - time-averaged filter estimation error for each robot and filter
            + included if config.record_time_averaged_error is set to True 
            + result_data['e_avg'][robot_idx][filter_idx][var]
    ...
    parameters:
     ...
     s_p[r_idx] - uncertainty of initial robot position estimate in each component (std.dev. in m) for robot robots[r_idx] 
     s_v[r_idx] - uncertainty of initial robot velocity estimate in each component (std.dev. in m/s) for robot robots[r_idx]
     s_th[r_idx] - uncertainty of initial robot heading estimate (std.dev. in rad) for robot robots[r_idx]
     s_ab[r_idx] - uncertainty of initial accelerometer bias estimate in each axis (std.dev. in m/s^2) for robot robots[r_idx]
     s_wb[r_idx] - uncertainty of initial gyro bias estimate in each axis (std.dev. in rad/s) for robot robots[r_idx]
     
     Pr0[r_idx] - initial covariance of robot state estimate for robot robots[r_idx]
    ...
    configuration:
     ...
     record_final_error - if set to True, sim_run() returns a list of tuples result_data['e_N'] containing final state errors for all robots and filters
     record_time_averaged_error - if set to True, sim_run() returns a list of tuples result_data['e_avg'] containing time-averaged state errors for all robots and filters
    """
    ...
    class parameters(sim.parameters):
        def __init__(self):
            ...
            self.s_p: list[float]
            self.s_v: list[float]
            self.s_th: list[float]
            self.s_ab: list[float]
            self.s_wb: list[float]
            
            self.Pr0: list[np.typing.NDArray]
    ...
    class configuration(sim.configuration):
        def __init__(self):
            ...
            self.record_final_error: bool 
            self.record_time_averaged_error: bool 

    def __init__(self, params: GPS_only_INS_sim.parameters, config: GPS_only_INS_sim.configuration, 
                 robot_data: list[tuple[simple_quad, INS_robot_state]]):
        # override parent class types
        self.params: GPS_only_INS_sim.parameters
        self.config: GPS_only_INS_sim.configuration
        
        super().__init__(params, config)
        
        # unpack robot data
        self.robots: list[simple_quad] = []
        self.robot_inivals: list[INS_robot_state] = []
    ...
    def get_var_attributes(cls):
        ...
        # information for additional result variables
        info.update({
            'e_N': {
                'var_title': 'final error',
                'tab_title': 'final error'
            },
            'e_avg': {
                'var_title': 'time-averaged error',
                'tab_title': 'time-avgd error'
            }
        })
    ...
    def sim_run(self, run_id: int, enable_replay: bool):
        ...
        if self.config.record_final_error:
            result_data['e_N'] = []

        if self.config.record_time_averaged_error:
            result_data['e_avg'] = []
        ...
        for r_idx, robot in enumerate(self.robots):
            ...
            for f_idx, (f, s0, P0) in enumerate(zip(robot.filters, s0s, P0s)):
                ...
                # compute initial filter errors
                if self.config.record_final_error or self.config.record_time_averaged_error:
                    true_state = self._get_true_state(r_idx, f_idx, 0, robot.s_traj[0])
                    f.compute_state_error(true_state, accumulate = self.config.record_time_averaged_error)
        ...
        while not done:
            ...
            for r_idx, robot in enumerate(self.robots):
                for f_idx, f in enumerate(robot.filters):
                    ...
                    if self.config.record_final_error or self.config.record_time_averaged_error:
                        true_state = self._get_true_state(r_idx, f_idx, t, robot.s_traj[t])
                        f.compute_state_error(true_state, accumulate = self.config.record_time_averaged_error)
        ...
        if not self.config.precompute_robot_trajectories:
            for robot in self.robots:
                ...
                result_data['waypose_steps'].append(robot.waypose_steps)
        ...
        # record global error measures
        for robot in self.robots:
            if self.config.record_final_error:
                el = []
                for f in robot.filters:
                    el.append(f.se)
                result_data['e_N'].append(tuple(el))

            if self.config.record_time_averaged_error:
                el = []
                for f in robot.filters:
                    e = {}
                    for var, value in f.cse.items():
                        e[var] = value / (robot.T + 1)
                    el.append(e)
                result_data['e_avg'].append(tuple(el))
    ...
    def replay(self, result_data: dict, record_pos_error_ellipsoid: bool):
        ...
        for r_idx, robot in enumerate(self.robots):
            ...
            if self.config.precompute_robot_trajectories:
                s_traj = robot.s_traj 
                waypose_steps = robot.waypose_steps
            else:
                s_traj = result_data['s_traj'][r_idx]
                waypose_steps = result_data['waypose_steps'][r_idx]
            ...
            for f_idx, f in enumerate(robot.filters):
                ...
                if record_pos_error_ellipsoid:
                    evals, evecs = linalg.eigh(getattr(f.P, 'Pr')[0:3, 0:3])
                    cov_s_traj_l.append([np.sqrt(evals)])
                    cov_q_traj_l.append([util.Rot2quat(np.ascontiguousarray(evecs))])
            ...
            for t in range(1, robot.T + 1 if self.config.precompute_robot_trajectories else len(result_data['s_traj'][r_idx])):
                for f_idx, f in enumerate(robot.filters):
                    ...
                    if record_pos_error_ellipsoid:
                        evals, evecs = linalg.eigh(getattr(f.P, 'Pr')[0:3, 0:3])
                        cov_s_traj_l[f_idx].append(np.sqrt(evals))
                        cov_q_traj_l[f_idx].append(util.Rot2quat(np.ascontiguousarray(evecs)))
    ...
    def _get_true_state(self, r_idx: int, f_idx: int, t: int, s: INS_robot_state):
    ...
    def _get_filter_initial_data(self, r_idx):
            ...
            s_ini_vals = {
                'p': robot.s_traj[0].p + self.params.s_p[r_idx] * np.random.randn(3, 1),
                'v': robot.s_traj[0].v + self.params.s_v[r_idx] * np.random.randn(3, 1),
                'q': util.ang_vel2quat((util.quat2angle(robot.s_traj[0].q) + self.params.s_th[r_idx] * np.random.randn()) * util.quat2axis(robot.s_traj[0].q), 1.0),
                'ab': robot.s_traj[0].ab + self.params.s_ab[r_idx] * np.random.randn(3, 1),
                'wb': robot.s_traj[0].wb + self.params.s_wb[r_idx] * np.random.randn(3, 1),
            }

            P_ini_vals = {
                'Pr': self.params.Pr0[r_idx],
            }
            ...
            for f in robot.filters:
                ...
                # initial covariance
                if f.__class__ in [GPS_only_INS_EKF]:
                    P0 = INS_filter_covariance(P_ini_vals['Pr'])
                elif f.__class__ in [GPS_only_INS_EqF]:
                    P0 = INS_EqF_covariance(P_ini_vals['Pr'])
                else:
