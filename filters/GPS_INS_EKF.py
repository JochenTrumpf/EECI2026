...
class GPS_INS_EKF(filter):
    ...
    parameters:
     ...
     R_INS - estimate of covariance of INS measurement error (9x9)
           - 0:3 estimate of covariance of GPS measurement y_gps (3x3)
           - 3:6 estimate of covariance of gravity direction measurement y_a (3x3)
           - 6:9 estimate of covariance of magnetic field direction measurement y_m (3x3)
    ...
    configuration:
     apply_INS_update_without_GPS - if set to True, update() will apply an update based only on INS measurements if no GPS measurement was received
                                    if set to False, update() will discard measurements that do not contain a GPS measurement
    ...
    class configuration(filter.configuration):
        def __init__(self):
            self.apply_INS_update_without_GPS: bool 
    ...
    def __init__(self, params: GPS_INS_EKF.parameters, config: GPS_INS_EKF.configuration):
        # override parent class types
        self.params: GPS_INS_EKF.parameters
        self.config: GPS_INS_EKF.configuration
    ...
    def update(self, m: dict, t_step: int, *_):
        ...
        # if GPS has dropped out only update robot attitude based on accelometer and magnetometer measurements
        if ('p' not in m) or (m['p'] is None):
            if self.config.apply_INS_update_without_GPS:
                self._update_only_attitude(m)
        else:
            self._update_full_INS(m)

    def _update_only_attitude(self, m: dict):
        ...
        y = np.block([[m['y_a']], [(np.eye(3) - m['y_a'] @ m['y_a'].T) @ m['y_m']]]) 
    
        a_a = - self.params.g / np.linalg.norm(self.params.g) # normalised acceleration measurement will be dominated by the negative of the direction of gravity
        a_m = (np.eye(3) - a_a @ a_a.T) @ self.params.m     # m = 3x1 magnetic field vector of area in local frame 
    
        q_w = self.s.q[0, 0]
        q_v = self.s.q[1:4, :]
        h_qa = np.block([2*q_w*a_a - 2*quat_utils.vec2skew(q_v) @ a_a,
                         2*q_w*quat_utils.vec2skew(a_a) + 2*(q_v.T @ a_a * np.eye(3) + q_v @ a_a.T - a_a @ q_v.T)]) #3x4
        h_qm = np.block([2*q_w*a_m - 2*quat_utils.vec2skew(q_v) @ a_m,
                         2*q_w*quat_utils.vec2skew(a_m) + 2*(q_v.T @ a_m * np.eye(3) + q_v @ a_m.T - a_m @ q_v.T) ]) #3x4
        H_x = np.block([[np.zeros((3,6)), h_qa, np.zeros((3,6))], [np.zeros((3,6)), h_qm, np.zeros((3,6))]]) #6x16
        H = H_x @ X_dx # 6x15

        K = self.P.Pr @ H.T @ np.linalg.inv(H @ self.P.Pr @ H.T + self.params.R_INS) # 15x9
    
        R = quat_utils.quat2Rot(self.s.q) 
        h_x = np.block([[R.T @ a_a],[R.T @ a_m]])
        dx = K @ (y - h_x)
    
        # P_new = (np.eye(6) - K @ H) @ P
        # More stable Joseph form (I − KH) P (I − KH)^T + K V K^T
        self.P.Pr = (np.eye(15) - K @ H) @ self.P.Pr @ (np.eye(15) - K @ H).T + K @ self.params.R_INS @ K.T
    ...
    def _update_full_INS(self, m: dict):
        X_dx = np.block([[np.eye(6), np.zeros((6,9))],
                         [np.zeros((4,6)), .5*quat_utils.left_mat(self.s.q) @ np.block([[np.zeros((1,3))],[np.eye(3)]]), np.zeros((4,6))],
                         [np.zeros((6,9)), np.eye(6)]]) #16x15
        y = np.block([[m['p']], [m['y_a']], [(np.eye(3) - m['y_a'] @ m['y_a'].T) @ m['y_m']]]) 
    
        a_a = self.params.g / np.linalg.norm(self.params.g) # normalised acceleration measurement will be dominated by the direction of gravity
        a_m = (np.eye(3) - a_a @ a_a.T) @ self.params.m     # m = 3x1 magnetic field vector of area in local frame 
        ...
        h_qm = np.block([2*q_w*a_m - 2*quat_utils.vec2skew(q_v) @ a_m,
                         2*q_w*quat_utils.vec2skew(a_m) + 2*(q_v.T @ a_m * np.eye(3) + q_v @ a_m.T - a_m @ q_v.T)]) #3x4
        H_x = np.block([[np.eye(3), np.zeros((3,13))], [np.zeros((3,6)), h_qa, np.zeros((3,6))], [np.zeros((3,6)), h_qm, np.zeros((3,6))]]) #9x16
        H = H_x @ X_dx # 9x15

        K = self.P.Pr @ H.T @ np.linalg.inv(H @ self.P.Pr @ H.T + self.params.R_INS) # 15x9
    
        R = quat_utils.quat2Rot(self.s.q) 
        h_x = np.block([[self.s.p],[R.T @ a_a],[R.T @ a_m]])
        dx = K @ (y - h_x)
